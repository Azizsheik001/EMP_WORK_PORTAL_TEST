...
export default function DashboardView({
  ...
  myClockStatus, pendingLeaveCount, allUsers, shiftChangeRequests, allShiftChangeRequests, compOffSummary, apiShifts,
}) {
  ...
  const myShiftToday = useMemo(() => {
    if (!apiShifts || !currentUser) return null;
    const targetDate = myClockStatus?.active_shift_date || myClockStatus?.shift_date;
    if (!targetDate) return null;
    return apiShifts.find(s => (s.user_id === currentUser.id || s.employee_id === currentUser.id) && s.shift_date === targetDate);
  }, [apiShifts, currentUser, myClockStatus]);
  ...
  const timeRemainingStr = useMemo(() => {
    ...
    const [shStr, smStr] = myShiftToday.shift_start_time.split(':');
    const sh = parseInt(shStr, 10);
    const sm = parseInt(smStr, 10);
    
    const [y, month, d] = myShiftToday.shift_date.split('-');
    
    const isOvernight = (eh * 60 + em) <= (sh * 60 + sm);
    
    // Create Date object for end time in the "local" context of the timezone
    const endTzDate = new Date(parseInt(y, 10), parseInt(month, 10) - 1, parseInt(d, 10), eh, em, 0);
    if (isOvernight) {
      endTzDate.setDate(endTzDate.getDate() + 1);
    }
    ...
    const currentHour = parseInt(p.hour, 10) % 24;
    const nowTzDate = new Date(parseInt(p.year, 10), parseInt(p.month, 10) - 1, parseInt(p.day, 10), currentHour, parseInt(p.minute, 10), parseInt(p.second, 10));
    
    const diffMs = endTzDate.getTime() - nowTzDate.getTime();
    ...
    if (h > 0) return `${h}h ${mRemaining}m remaining`;
    return `${mRemaining}m remaining`;
  }, [isClockedIn, myShiftToday, currentUser]);

  const [, setTick] = useState(0);
  useEffect(() => {
    if (!timeRemainingStr || timeRemainingStr === 'Shift ended') return;
  ...
  const onLeaveIds = useMemo(() => {
    const ids = new Set();
    if (!statsSource || !Array.isArray(statsSource)) return ids;
    statsSource.forEach((r) => {
      ...
      if (start <= actualToday && end >= actualToday) {
        ids.add(r.employeeId || r.employee_id);
      }
    });
    return ids;
  ...
  const todayRows = useMemo(() => {
    ...
    if (shouldFilterShifts && teamUserIdSet.size > 0) rows = rows.filter(r => teamUserIdSet.has(r.employeeId));
    
    return rows.map(r => {
      if (r.status === 'absent' || r.status === 'not_started' || r.status === 'current_not_logged_in' || r.status === 'off') {
        if (onLeaveIds.has(r.employeeId)) {
          return { ...r, status: 'leave' };
        }
      }
      return r;
    });
  }, [todayShifts, today, shouldFilterShifts, teamUserIdSet, adminIdSet, onLeaveIds]);
  ...
  const onLeaveTodayList = useMemo(() => {
    ...
    return onLeaveRequests.filter((r) => {
      ...
      name: r.employeeName,
      detail: { casual: 'CL', sick: 'SL', comp: 'Comp', loss_of_pay: 'LOP', work_from_home: 'WFH', work_from_office: 'WFO', national_holiday_comp_off: 'NHC', half_day: 'Half Day' }[r.leave_type] || r.leave_type || r.reason || '',
  ...
  const departmentCounts = useMemo(() => {
    ...
    teamUsers.forEach((u) => {
      ...
      if (u.is_active === false) return; // Ignore deactivated users
  ...
  const pendingApprovalList = useMemo(() => {
    ...
    // Use the broader src so we have all requests for admin/manager
    const src = allLeaveRequests && allLeaveRequests.length > 0
    ...
    // Deduplicate by id
    const seen = new Set();
    const deduped = src.filter(r => { if (seen.has(r.id)) return false; seen.add(r.id); return true; });

    const PENDING = ['pending_team_lead', 'pending_managers', 'pending_ceo'];

    return deduped
      .filter((r) => {
        const requesterId = r.employeeId || r.employee_id;
        // Never show own requests
        if (requesterId === currentUser?.id) return false;
        // Must be a pending status
        if (!PENDING.includes(r.status)) return false;
        ...
        // Admin or HR/Finance can see and action ALL pending stages
        if (userType === 'admin' || isHRorFinance) return true;

        const requester = allUsers?.find(u => u.id === requesterId);

        // Manager: only at pending_managers, and haven't already approved
        if (userType === 'manager') {
          if (r.status !== 'pending_managers') return false;
        ...
        // Team lead: only pending_team_lead requests where they are the assigned TL
        if (userType === 'team_lead') {
          if (r.status !== 'pending_team_lead') return false;
          return requester && (requester.team_lead_id === currentUser?.id || requester.id === currentUser?.id);
        }
        return false;
      })
      .sort((a, b) => String(a.requestedAt || a.requested_at || '').localeCompare(String(b.requestedAt || b.requested_at || '')))
      .slice(0, 5);
  }, [canApprove, leaveRequests, allLeaveRequests, currentUser?.id, userType]);
  ...
  // Upcoming approved leaves (today and future) — for admin/manager/team_lead
  const upcomingApprovedLeaves = useMemo(() => {
    if (!canApprove) return [];
    // Merge leaveRequests + allLeaveRequests, deduplicate by id
    const rawSrc = allLeaveRequests && allLeaveRequests.length > 0
      ? [...(leaveRequests || []), ...(allLeaveRequests || [])]
      : (leaveRequests || []);
    if (!Array.isArray(rawSrc) || rawSrc.length === 0) return [];
    const seen = new Set();
    const src = rawSrc.filter(r => { if (seen.has(r.id)) return false; seen.add(r.id); return true; });
    return src
      .filter((r) => {
      ...
      .slice(0, 8);
  }, [canApprove, leaveRequests, allLeaveRequests, currentUser?.id, actualToday]);

  // Employee's own upcoming leaves (for employee role dash view)
  const myUpcomingLeaves = useMemo(() => {
    ...
    return src
      .filter((r) => {
        const eid = r.employeeId || r.employee_id || r.user_id;
        if (eid !== currentUser?.id) return false;
        const end = String(r.end_date || r.start_date || '').slice(0, 10);
        return end >= actualToday;
      })
      .sort((a, b) => String(a.start_date || '').localeCompare(String(b.start_date || '')))
      .slice(0, 6);
  }, [canApprove, allLeaveRequests, leaveRequests, currentUser?.id, actualToday]);

  // Employee's approved leaves this year (for the Approved Leaves card)
  const myApprovedLeaves = useMemo(() => {
    ...
    return src
      ...
      .slice(0, 6);
  }, [canApprove, allLeaveRequests, leaveRequests, currentUser?.id]);
  ...
  const donutSegments = [
    ...
    { value: attendanceCounts.leave, color: '#8b5cf6', label: 'Leave' },
    ...
    <div className="space-y-6 max-w-7xl mx-auto w-full">
                    ...
                    <React.Fragment key={req.id}>
                      ...
                      >
                        <td className="px-4 py-2.5 font-medium whitespace-nowrap">
                          {formatName(req.employeeName) || 'Employee'}
      ...
      {!canApprove && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className={`rounded-xl border overflow-hidden ${card}`}>
            <div className={`flex items-center justify-between px-4 py-3 border-b ${borderColor}`}>
              <h2 className={`text-[10px] font-semibold uppercase tracking-wider ${subtleText}`}>My Upcoming Leaves</h2>
              <button type="button" onClick={() => onNavigate?.('approvals')}
                className="text-[11px] font-medium text-brand hover:text-brand-hover transition-colors">View all</button>
            </div>
            {myUpcomingLeaves.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-full text-xs">
                  <thead>
                    <tr className={isDark ? 'bg-slate-700/50' : 'bg-gray-50'}>
                      <th className={`text-left px-4 py-2.5 font-medium ${subtleText}`}>Type</th>
                      <th className={`text-left px-4 py-2.5 font-medium ${subtleText}`}>Dates</th>
                      <th className={`text-left px-2 py-2.5 font-medium ${subtleText}`}>Days</th>
                      <th className={`text-left px-2 py-2.5 font-medium ${subtleText}`}>Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {myUpcomingLeaves.map((req) => {
                      const startStr = String(req.start_date || '').slice(0, 10);
                      const endStr = String(req.end_date || req.start_date || '').slice(0, 10);
                      const isActive = startStr <= actualToday && endStr >= actualToday;
                      const st = (req.status || '').toLowerCase();
                      const statusColor = st === 'approved'
                        ? (isDark ? 'text-green-400' : 'text-green-600')
                        : st.startsWith('pending') ? (isDark ? 'text-amber-400' : 'text-amber-600')
                        : st === 'rejected' ? 'text-red-500' : subtleText;
                      const rowColor = isActive
                        ? (isDark ? 'bg-amber-900/30' : 'bg-amber-50')
                        : (isDark ? 'hover:bg-slate-700/40' : 'hover:bg-gray-50/80');
                      return (
                        <tr key={req.id} className={`border-t ${borderColor} ${rowColor} transition-colors`}>
                          <td className="px-4 py-2.5 font-medium whitespace-nowrap">
                            {{ casual: 'Casual Leave', sick: 'Sick Leave', comp: 'Comp Off', loss_of_pay: 'LOP', work_from_home: 'WFH', work_from_office: 'WFO', national_holiday_comp_off: 'NHC', half_day: 'Half Day' }[req.leave_type] || req.leave_type || '--'}
                            {isActive && <span className={`ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wide ${isDark ? 'bg-amber-800 text-amber-100' : 'bg-amber-500 text-white'}`}>Active</span>}
                          </td>
                          <td className={`px-4 py-2.5 whitespace-nowrap ${subtleText}`}>{formatLeaveDate(req.start_date, req.end_date)}</td>
                          <td className={`px-2 py-2.5 ${subtleText}`}>{req.total_days || '--'}</td>
                          <td className={`px-2 py-2.5 font-medium ${statusColor}`}>
                            {st === 'pending_team_lead' || st === 'pending_managers' || st === 'pending_ceo' ? 'Pending' : st.charAt(0).toUpperCase() + st.slice(1)}
                          </td>
                        </tr>
                      );
                    })}
          ...
          <div className={`rounded-xl border overflow-hidden ${card}`}>
            <div className={`flex items-center justify-between px-4 py-3 border-b ${borderColor}`}>
              <h2 className={`text-[10px] font-semibold uppercase tracking-wider ${subtleText}`}>Approved Leaves</h2>
              <button type="button" onClick={() => onNavigate?.('approvals')}
                className="text-[11px] font-medium text-brand hover:text-brand-hover transition-colors">View all</button>
            </div>
            {myApprovedLeaves.length === 0 ? (
              <div className="px-5 py-10 text-center">
                ...
                <p className={`text-xs ${subtleText}`}>No approved leaves this year</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full min-w-full text-xs">
                  ...
                  <tbody>
                    {myApprovedLeaves.map((req) => {
                      const startStr = String(req.start_date || '').slice(0, 10);
                      const endStr = String(req.end_date || req.start_date || '').slice(0, 10);
                      const isActive = startStr <= actualToday && endStr >= actualToday;
                      const isPast = endStr < actualToday;
                      const rowColor = isActive
                      ...
                      return (
                        <tr key={req.id} className={`border-t ${borderColor} ${rowColor} transition-colors`}>
                          <td className="px-4 py-2.5 font-medium whitespace-nowrap">
                            {{ casual: 'Casual Leave', sick: 'Sick Leave', comp: 'Comp Off', loss_of_pay: 'LOP', work_from_home: 'WFH', work_from_office: 'WFO', national_holiday_comp_off: 'NHC', half_day: 'Half Day' }[req.leave_type] || req.leave_type || '--'}
                            {isActive && <span className={`ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wide ${isDark ? 'bg-green-800 text-green-100' : 'bg-green-500 text-white'}`}>Active</span>}
                          </td>
                          <td className={`px-4 py-2.5 whitespace-nowrap ${isPast ? subtleText : ''}`}>{formatLeaveDate(req.start_date, req.end_date)}</td>
                          <td className={`px-2 py-2.5 font-semibold text-green-500`}>{req.total_days || '--'}</td>
                        </tr>
                      );
                    })}
                  </tbody>
      ...
      <div className={`rounded-xl border overflow-hidden ${card}`}>
        <div className="grid grid-cols-2 sm:grid-cols-4">
          {[
            { label: 'My Work', tab: 'my-work', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z', show: true },
            { label: 'Schedules', tab: 'schedules', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2', show: canApprove },
            { label: 'Team', tab: 'team', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z', show: true },
            { label: 'Assets', tab: 'asset-management', icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4', show: canApprove },
