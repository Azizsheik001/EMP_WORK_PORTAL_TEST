...
export default function UploadSchedules({ isDark, clients = [], allUsers = [] }) {
  ...
  const handleDownloadTemplate = useCallback(() => {
    const header = ['Employee Name', 'Employee ID', ...weekDates.map((d, i) => `${DAY_LABELS[i]} (${formatDateShort(d)})`)];
    const rows = employees.map((emp) => {
      const name = (emp.name || '').replace(/,/g, ' ');
      const empId = emp.employee_no || emp.id || '';
      return [name, empId, ...Array(7).fill('')].join(',');
    });
    ...
    a.href = url;
    a.download = `schedule-template-${clientName.replace(/\s+/g, '_')}-${startDate}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  }, [employees, weekDates, clientName, startDate]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      // Build assignments for all weeks
      ...
      for (let w = 0; w < totalWeeks; w++) {
        ...
        previewData.forEach((a) => {
          allAssignments.push({
            user_id: a.user_id,
            shift_date: addDays(a.shift_date, weekOffset),
  ...
  }, [previewData, selectedClient, totalWeeks, startDate]);
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                className={selectClass}
              />
              <p className="text-xs text-brand font-medium mt-1">{getWeekLabel(startDate)}</p>
            </div>
          </div>
