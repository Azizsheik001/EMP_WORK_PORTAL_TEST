...
export default function LoginPage({ onLogin }) {
  ...
  const handleForceResetDone = () => {
    ...
    if (forceResetData) {
      ...
      // Mark this user as needing NDA — they are a new employee doing first-time setup
      try {
        const existing = localStorage.getItem(`ags_nda_status_${normalized.id}`);
        // Only set if they haven't already signed (don't override completed/pending_admin status)
        if (!existing) {
          localStorage.setItem(`ags_nda_status_${normalized.id}`, 'pending_user');
        }
      } catch {}
      onLogin(normalized);
    }
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder="••••••••"
              className="w-full rounded-lg border-2 border-gray-400 dark:border-slate-500 bg-gray-50 dark:bg-slate-700 px-3 py-2.5 pr-10 text-gray-900 dark:text-white placeholder-gray-500 focus:ring-2 focus:ring-brand focus:border-brand"
            />
            <button
              type="button"
