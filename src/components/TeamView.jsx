...
function MyTeamView({ isDark, currentUser, allUsers, shiftRows, leaveRequests }) {
  ...
  // Get teammates: scoped to same department (or same client if no dept), exclude self and admin
  const teammates = useMemo(() => {
    if (!allUsers) return [];
    const myTLId = currentUser?.team_lead_id;
    const myMgrId = currentUser?.manager_id;
    return allUsers.filter((u) => {
      if (u.id === myId || u.role === 'admin' || u.is_active === false) return false;
      // If current user has a department, only show people from the same department
      if (myDeptId) {
        const sameDept = u.department_id === myDeptId
          || (Array.isArray(u.department_ids) && u.department_ids.includes(myDeptId));
        // Also always include the assigned team lead and manager regardless of their dept
        const isMyTL = myTLId && u.id === myTLId;
        const isMyMgr = myMgrId && u.id === myMgrId;
        return sameDept || isMyTL || isMyMgr;
      }
      // Fallback: if user has no department, scope by client_id
      if (myClientId) return u.client_id === myClientId;
      return false;
    });
  }, [allUsers, myId, myDeptId, myClientId, currentUser]);
      ...
      {manager && (
        <div className={`rounded-xl border p-4 ${card}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className={`font-medium text-sm ${isDark ? 'text-white' : 'text-gray-900'}`}>{formatName(manager.name)}</p>
      ...
      {teamLead && (
        <div className={`rounded-xl border p-4 ${card}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className={`font-medium text-sm ${isDark ? 'text-white' : 'text-gray-900'}`}>{formatName(teamLead.name)}</p>
            ...
            <div className={`divide-y ${isDark ? 'divide-slate-700' : 'divide-gray-100'}`}>
              ...
              return (
                <div key={t.id} className={`flex items-center gap-3 px-4 py-3 ${isDark ? 'hover:bg-slate-700/40' : 'hover:bg-gray-50'} transition-colors`}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className={`font-medium text-sm truncate ${isDark ? 'text-white' : 'text-gray-900'}`}>{formatName(t.name)}</span>
                      ...
                      <div key={dept.id || 'null'} className={`rounded-xl border p-4 ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-gray-200'}`}>
                        ...
                        {dept.unassigned_employees?.length > 0 && (
                          <p className={`text-xs mt-2 ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>
                            {dept.unassigned_employees.length} {dept.clients?.length === 0 ? 'employee(s)' : 'employee(s) not in a client'}
                          </p>
                        )}
                      </div>
                    ))}
                    ...
                    {selectedDeptData.unassigned_employees.length > 0 && (
                      <div className="mt-4">
                        <p className={`text-sm font-semibold mb-2 ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>
                          Employees ({selectedDeptData.unassigned_employees.length})
                        </p>
                        <EmployeeTable
