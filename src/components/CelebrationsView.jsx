...
const TABS = [
  ...
  { id: 'month', label: 'Upcoming (1 Month)' },
];
...
export default function CelebrationsView({ isDark, currentUser }) {
  ...
  const fetchCelebrations = useCallback(async () => {
    ...
    try {
      const promises = [
        ...
        api.celebrations?.upcoming?.()?.catch?.(() => ({ celebrations: [] })) ??
          Promise.resolve({ celebrations: [] }),
      ];

      const [todayRes, upcomingRes] = await Promise.all(promises);

      setTodayData(todayRes?.celebrations || todayRes?.birthdays || []);
      setMonthData(upcomingRes?.celebrations || upcomingRes?.birthdays || []);
    } catch (e) {
  ...
  const currentData = activeTab === 'today' ? todayData : monthData;
