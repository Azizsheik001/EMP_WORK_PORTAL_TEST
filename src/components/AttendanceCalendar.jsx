 ...
 * @returns {'P'|'A'|'L'|'O'|'H'|'-'}
 */
function getDayStatus(dateStr, shift, leaveType, todayStr, isMandatoryHoliday) {
  ...
  // Approved leave
  if (leaveType) {
    if (leaveType === 'work_from_home') return 'W';
    if (leaveType === 'work_from_office') return 'WO';
    if (leaveType === 'half_day') return 'H';
    if (!clockedIn) return 'L';
  }

  // If employee clocked in, calculate Present / Half Day
  if (clockedIn) {
  ...
  if (isMandatoryHoliday) {
    return 'HD'; // Holiday overrides Absent/Off/Upcoming
  }

  // No shift assigned or explicitly marked as off = off day 
  if (!shift || shift.is_off || !shift.shift_start_time || !shift.shift_end_time) {
    return 'O';
  }
  ...
  if (isToday) {
    ...
    if (nowMin < shiftStartMin + 60 && !isNaN(sh)) return '-';
  }
...
const STATUS_CONFIG = {
  ...
  W: { label: 'WFH',     letter: 'W', bg: 'bg-indigo-100 dark:bg-indigo-900/40', text: 'text-indigo-700 dark:text-indigo-300', dot: 'bg-indigo-500' },
  WO: { label: 'WFO',    letter: 'WO', bg: 'bg-teal-100 dark:bg-teal-900/40', text: 'text-teal-700 dark:text-teal-300', dot: 'bg-teal-500' },
  O: { label: 'Off',     letter: 'O', bg: 'bg-gray-100 dark:bg-gray-800/60',  text: 'text-gray-500 dark:text-gray-400',   dot: 'bg-gray-400' },
  H: { label: 'Half Day',letter: 'HD', bg: 'bg-amber-100 dark:bg-amber-900/40',text: 'text-amber-700 dark:text-amber-300', dot: 'bg-amber-500' },
  HD: { label: 'Holiday',letter: 'H', bg: 'bg-purple-100 dark:bg-purple-900/40', text: 'text-purple-700 dark:text-purple-300', dot: 'bg-purple-500' },
...
export default function AttendanceCalendar({ isDark, currentUser, onNavigate, showToast }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [shiftData, leaveData, holidayData] = await Promise.all([
        api.shifts({ from, to }),
  ...
  // Build a map of dates that have approved leave for the selected user -> leave_type
  const leaveDates = useMemo(() => {
    ...
    for (const lr of leaveRequests) {
      ...
      while (cursor <= limit) {
        dates.set(cursor.toISOString().slice(0, 10), lr.leave_type);
  ...
  const calendarWeeks = useMemo(() => {
    ...
    // Determine selected user's calendars
    const u = users.find(usr => usr.id === selectedUserId) || currentUser || {};
    const tz = (u.work_timezone || '').toLowerCase();
    const nameLower = (u.name || '').toLowerCase();
    const isUS = tz.startsWith('america/') || ['us', 'cst', 'est', 'pst', 'mst'].includes(tz) || ['jaswanthi', 'rohan', 'girish'].some(n => nameLower.includes(n));
    const userCals = isUS ? ['US', 'All'] : ['IND', 'All'];
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const holidayMatch = holidays.find(h => h.holiday_date === dateStr && userCals.includes(h.calendar));
      let isMandatoryHoliday = false;
      if (holidayMatch) {
        if (holidayMatch.calendar === 'US' || holidayMatch.calendar === 'IND') {
          isMandatoryHoliday = true;
        } else {
          isMandatoryHoliday = !holidayMatch.is_optional;
        }
      }

      const status = getDayStatus(dateStr, shift, leaveType, todayStr, isMandatoryHoliday);
    ...
    return weeks;
  }, [year, month, shiftMap, leaveDates, holidays, users, selectedUserId, currentUser]);

  // Summary counts
  const summary = useMemo(() => {
    const counts = { P: 0, A: 0, L: 0, W: 0, O: 0, H: 0, HD: 0 };
        ...
        <div className={`rounded-xl border ${cardBg} p-4`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3">
            {[
              ...
              { key: 'W', label: 'WFH',      color: 'indigo' },
              { key: 'O', label: 'Off',      color: 'gray' },
              { key: 'H', label: 'Half Day', color: 'amber' },
              { key: 'HD', label: 'Holiday', color: 'purple' },
            ].map(({ key, label, color }) => {
              const bgMap = {
                ...
                indigo: isDark ? 'bg-indigo-900/20 border-indigo-800/40' : 'bg-indigo-50 border-indigo-200',
                ...
                purple: isDark ? 'bg-purple-900/20 border-purple-800/40' : 'bg-purple-50 border-purple-200',
              };
              const numMap = {
