...
import MyNotificationsModal from './components/MyNotificationsModal';
...
import NdaFormPage from './components/NdaFormPage';
import DocumentFormPage from './components/DocumentFormPage';
import ShreeNdaReviewPage from './components/ShreeNdaReviewPage';
import NdaTemplateBuilderPage from './components/NdaTemplateBuilderPage';
import HRDocumentsView from './components/HRDocumentsView';
...
const NAV_ITEMS = [
  ...
  { id: 'employee-data', label: 'Employee Data', icon: 'people', roles: ['admin'] },
  ...
  { id: 'documents', label: 'Documents', icon: 'documents', roles: ['admin', 'manager', 'team_lead', 'employee'] },
  {
    id: 'hr',
    label: 'HR',
    icon: 'hr',
...
export default function App() {
  ...
  const [ndaDoneKey, setNdaDoneKey] = useState(0);
  const [pendingNdaRequest, setPendingNdaRequest] = useState(null);
  const [checkingNdaRequest, setCheckingNdaRequest] = useState(false);
  const [showNdaBuilderInDocs, setShowNdaBuilderInDocs] = useState(false);
  ...
  useEffect(() => {
    if (!currentUser || !hasApi()) {
      setPendingNdaRequest(null);
      setCheckingNdaRequest(false);
      return;
    }
    ...
    const skipNdaCheck =
      ...
      pathname === '/shree-nda-review' ||
      pathname === '/nda-template-builder';
    ...
    let alive = true;
    setCheckingNdaRequest(true);

    api.nda
      .getMyPending(currentUser.email)
      .then((data) => {
        if (!alive) return;
        setPendingNdaRequest(data?.nda || null);
      })
      .catch(() => {
        if (!alive) return;
        setPendingNdaRequest(null);
  ...
  }, [currentUser?.id, currentUser?.email, ndaDoneKey]);
  ...
  }, [currentUser, activeTab, week, dateFrom, dateTo, clientId, shiftsRefreshKey]);
  ...
  useEffect(() => {
    ...
    const userTz = currentUser.work_timezone || browserTimezone || 'Asia/Kolkata';
  ...
  const shiftRowsForTable = useMemo(() => {
    ...
    const allApiRows = buildShiftRowsFromApi(apiShifts, { searchQuery, clientId: clientId || null, today: dateFrom || undefined, adminIds, leaveRequests });
    ...
    return rows;
  }, [apiShifts, searchQuery, clientId, assignedEmployeesByClient, allUsers, selectedDepartment, userDeptMap, userDeptNameMap, userClientMap, leaveRequests]);
  ...
  const refreshClockStatus = useCallback(async () => {
    ...
    try {
      const browserTimezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
      const userTimezone = currentUser.work_timezone || browserTimezone || 'Asia/Kolkata';
  ...
  const handleApproveShiftChange = useCallback(async (requestId) => {
    if (hasApi()) {
      try {
        ...
        fetchShiftChangeRequests();
        setShiftsRefreshKey((k) => k + 1); // re-fetch apiShifts so Team/MyWork show updated shift time
  ...
  const pendingLeaveCount = (() => {
    ...
    const canSeeRequest = (r) => {
      const requesterId = r.employeeId || r.employee_id || r.user_id;
      if (requesterId === userId) return false;
      if ((r.approvalChain || r.approval_chain || []).some((a) => a.userId === userId || a.user_id === userId)) return false;
      
      const st = r.status || '';
      if (type === 'admin') return ['pending_team_lead', 'pending_managers', 'pending_ceo'].includes(st);
      
      if (isHRorFinance && ['pending_team_lead', 'pending_managers', 'pending_ceo'].includes(st)) return true;

      const requester = allUsers.find(u => u.id === requesterId);
      if (type === 'team_lead' && st === 'pending_team_lead') {
        return requester && (requester.team_lead_id === userId || requester.id === userId);
      }
      if (type === 'manager' && st === 'pending_managers') {
        return requester && (requester.manager_id === userId || requester.id === userId);
      }
      return false;
    };

    const leaveCount = leaveRequests.filter(canSeeRequest).length;
    const unacknowledgedCount = type === 'admin' || isHRorFinance
      ? leaveRequests.filter((r) => r.status === 'approved' && !r.acknowledgedBy && (r.employeeId || r.employee_id) !== userId).length
      : 0;
    const shiftChangeCount = shiftChangeRequests.filter(canSeeRequest).length;
    const unacknowledgedShiftCount = type === 'admin' || isHRorFinance
      ? shiftChangeRequests.filter((r) => r.status === 'approved' && !(r.seen_by || []).includes(userId) && (r.user_id || r.employee_id) !== userId).length
      : 0;
  ...
  const clockedInAtFromApi = hasApi() && myClockStatus?.clocked_in_at
    ? new Date(myClockStatus.clocked_in_at).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: currentUser?.work_timezone || 'Asia/Kolkata' })
    : null;
    ...
    return NAV_ITEMS
      .filter((item) => {
        ...
        if (item.id === 'documents') {
          ...
          if (role === 'manager') {
            const dept = (currentUser.department_name || '').toLowerCase();
            if (dept === 'hr' || dept === 'finance' || dept === 'human resources') return true;
          }
          return false;
        }
        return true;
          ...
          return {
            ...
            children: item.children.filter((c) => {
              ...
              if (role === 'employee') {
                ...
                if (isHrOrFinance && ['asset-management', 'budgeting', 'reports', 'leave-report', 'dinner-tracker', 'cab-drops'].includes(c.id)) {
                  return true;
                }
              }
              return false;
  ...
  const allowedTabIds = useMemo(() => {
    ...
    // changelog and celebrations are accessed via buttons/panels, not sidebar nav
    set.add('changelog');
    set.add('celebrations');
    return set;
  ...
  const isNdaTemplateBuilderRoute =
    pathname === '/nda-template-builder' ||
    activeTab === 'nda-template-builder';

  const isNdaFormRoute =
    pathname.startsWith('/nda-form') ||
    activeTab === 'nda-form';

  const isDocumentFormRoute =
    pathname === '/document-form' ||
    activeTab === 'document-form';

  const isShreeNdaRoute =
    pathname.startsWith('/shree-nda-review');

  if (isNdaTemplateBuilderRoute) {
    return (
      <NdaTemplateBuilderPage
  ...
  if (checkingNdaRequest && !isNdaFormRoute && !isDocumentFormRoute && !isShreeNdaRoute) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-slate-900">
        <div className="text-center">
          <div className="inline-block animate-spin rounded-full h-10 w-10 border-b-2 border-green-600 mb-4"></div>
          <p className="text-gray-600 dark:text-gray-300">Checking NDA status...</p>
        </div>
      </div>
    );
  }

  // Full-screen NDA flow — show ONLY the NDA page, no sidebar or header.
  // New employees must complete NDA once before reaching dashboard.
  if (isNdaFormRoute || pendingNdaRequest) {
    return (
      <NdaFormPage
        currentUser={currentUser}
        isDark={isDark}
        ndaId={pendingNdaRequest?.id}
        onSubmit={() => {
          setPendingNdaRequest(null);
          setNdaDoneKey((k) => k + 1);
          window.history.pushState({}, '', '/');
          setActiveTab('dashboard');
          showToast('NDA submitted successfully');
        }}
      />
    );
  }
    ...
    <div className="h-screen flex overflow-hidden bg-gray-50 dark:bg-slate-900 text-gray-900 dark:text-white">
      ...
      >
        <div className={`border-b border-gray-200 dark:border-slate-700 flex items-center ${sidebarCollapsed ? 'lg:justify-center p-3' : 'justify-between p-4'}`}>
          <div className={`flex items-center gap-2 min-w-0 ${sidebarCollapsed ? 'lg:justify-center' : ''}`}>
            <img src="/leftpanel.png" alt="AGS Logo" className="h-12 w-auto object-contain flex-shrink-0" />
            <div className={`min-w-0 ${sidebarCollapsed ? 'lg:hidden' : ''}`}>
              <span className="font-semibold text-sm text-brand block leading-tight truncate">AGS Workforce Portal</span>
              <span className="text-[10px] text-gray-400 dark:text-gray-500 leading-tight truncate block">Workforce Management and Employee Scheduling</span>
            </div>
          </div>
          <button
            type="button"
        ...
        <nav className={`flex-1 py-2 overflow-y-auto overflow-x-hidden ${sidebarCollapsed ? 'lg:px-0' : 'px-2'}`}>
          ...
          <div className={`pt-2 mt-2 border-t border-gray-200 dark:border-slate-700 ${sidebarCollapsed ? 'lg:mx-0' : ''}`}>
            ...
            <button
              ...
              className={`w-full text-left text-sm font-medium transition-all flex items-center gap-2.5 border-l-4 border-transparent bg-red-500 hover:bg-red-600 text-white ${
                sidebarCollapsed
                  ? 'lg:flex-col lg:items-center lg:gap-1 lg:py-3 lg:px-2 lg:text-[10px] px-3 py-2 rounded-lg mt-2'
            ...
            >
              ...
              <span className={`${sidebarCollapsed ? 'lg:text-center lg:leading-tight lg:truncate lg:w-full' : ''} truncate`}>
                Logout
              </span>
            </button>
          </div>
      ...
      <main className="flex-1 flex flex-col min-w-0" id="main-content">
        ...
        <div className="flex-1 overflow-auto p-4 sm:p-6">
          ...
          {activeTab === 'employee-data' && (
            <ErrorBoundary inline fallbackMessage="Could not load Employee Data. Please try again.">
              <EmployeeDataView
                isDark={isDark}
                currentUser={currentUser}
              />
          ...
          {activeTab === 'documents' && (
            <ErrorBoundary inline fallbackMessage="Could not load Documents. Please try again.">
              {showNdaBuilderInDocs ? (
                <NdaTemplateBuilderPage
                  currentUser={currentUser}
                  isDark={isDark}
                  onBack={() => setShowNdaBuilderInDocs(false)}
                  editDoc={typeof showNdaBuilderInDocs === 'object' ? showNdaBuilderInDocs : null}
                  showToast={showToast}
                />
              ) : (
                <HRDocumentsView
                  ...
                  currentUser={currentUser}
                  allUsers={allUsers}
                  onNavigateNda={() => setActiveTab('nda-form')}
                  onEditNdaForm={(doc) => setShowNdaBuilderInDocs(doc || true)}
                />
              )}
            </ErrorBoundary>
          )}
          ...
          {activeTab === 'user-management' && currentUser?.type !== 'employee' && (
            <ErrorBoundary inline fallbackMessage="Could not load User Management. Please try again.">
              <UserManagementView
                ...
                onEditNdaForm={() => {
                  setShowNdaBuilderInDocs(true);
                  setActiveTab('hr-documents');
                }}
              />
            </ErrorBoundary>
          )}
      ...
      <RightPanel
        ...
        onChangelogClick={() => { setRightPanelOpen(false); setActiveTab('changelog'); }}
        ...
        onMyNotificationsClick={() => setMyNotificationsOpen(true)}
      />
      <MyNotificationsModal isOpen={myNotificationsOpen} onClose={() => setMyNotificationsOpen(false)} isDark={isDark} />
      {userMgmtOpen && (
        <UserManagementModal
